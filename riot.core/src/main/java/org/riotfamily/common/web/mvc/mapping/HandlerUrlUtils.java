...
public final class HandlerUrlUtils {
	...
	public static HandlerUrlResolverIntf getUrlResolver(HttpServletRequest request) {
		return getUrlResolver(RequestContextUtils.getWebApplicationContext(request));
	}
	
	public static HandlerUrlResolverIntf getUrlResolver(BeanFactory lbf) {
		if (lbf.containsBean("handlerUrlResolver")) {
			return SpringUtils.getBean(lbf,	"handlerUrlResolver", 
					HandlerUrlResolverIntf.class);
		}
		return SpringUtils.beanOfType(lbf, HandlerUrlResolverIntf.class);
	}
